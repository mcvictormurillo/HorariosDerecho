...
class ProfesorController extends Controller
{
    ...
    public function index(Request $request)
    {
    	if($request)
    	{
    		...
    		$profesores=DB::table('profesor')->where('NOMBREPROF','LIKE','%'.$query.'%')
    		//->where('telefono','=','1')
    		->orderBy('NOMBREPROF','asc')
    		->paginate(7); 
    ...
    {//almacena el objeto del modelo profesor en nuestra tabla profesor de la bd
        ...
        //ojo estos son los campos de la base de datos
    	$profesor->NOMBREPROF=$request->get('nombre');
    	$profesor->TELEFONOPROF=$request->get('telefono');
    	$profesor->CORREOPROF=$request->get('correo');
    	$profesor->ESTADOPROF='1';
    	$profesor->save();
